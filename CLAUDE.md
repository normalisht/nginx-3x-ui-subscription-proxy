...
| `EXTERNAL_SUBSCRIPTION_HEADERS` | Необязательные HTTP-заголовки, которые бэкенд сам подставляет при запросе к `EXTERNAL_SUBSCRIPTIONS` (не заголовки реального клиента, а имитация — например `User-Agent` и `Hwid` как у обычного VPN-клиента). Формат: `Key: Value`, несколько заголовков разделяются `;;` либо переносом строки, например `User-Agent: Happ/2.4.1 (Android 13; SDK 33);;Hwid: 1234567890abcdef` |
