...
CERT_DIR="/etc/letsencrypt/live/$SITE_HOST_SECOND"
CERT_PATH="$CERT_DIR/fullchain.pem"
KEY_PATH="$CERT_DIR/privkey.pem"

if [ ! -f "$CERT_PATH" ] || [ ! -f "$KEY_PATH" ]; then
    echo "Сертификаты для $SITE_HOST_SECOND не найдены или неполны. Получаем новые..."
    certbot certonly --webroot \
        -w $WEBROOT_PATH \
        -d $SITE_HOST_SECOND \
        --email $EMAIL \
    ...
    echo "Сертификаты для $SITE_HOST_SECOND уже существуют."
    certbot renew --webroot -w $WEBROOT_PATH --non-interactive --deploy-hook "nginx -s reload"
fi
